...
public class ExpandableListAdapter extends BaseExpandableListAdapter {
    ...
    public View getChildView(int i, int i1, boolean b, View view, ViewGroup viewGroup) {
        ...
        List<Note> ItemByTitle = getAppDatabase(view.getContext()).noteDao().GetAllByTitle((String)getGroup(i));
        for (int j = 0; j < ItemByTitle.size(); j++) {
            if((ItemByTitle.get(j).getIsDone())==1)
            {
                if (title != null) {
         ...
         else  if((ItemByTitle.get(j).getIsDone())==0)
            {
                title.setText(child_title);
            }
